...
{
   /// <summary>
    /// Cache Repository for UsersMongo that uses the Decorator pattern to add caching functionality to the MongoDB repository.
    /// </summary>
    public sealed class CachingUserMongoRepository : IMongoDbRepository<UserModel>
    {
        private readonly UserMongoRepository _decorated;
        private readonly IDistributedCache _distributedCache;
        private const int ITEMS_EXPIRATION_TIME = 5;
        private const int ITEM_BY_ID_EXPIRATION_TIME = 2;

        private const string ITEMS_KEY = "user-mongo-all";
        private const string ITEM_BY_ID_KEY = "user-mongo-{0,0}"; 
        
        public CachingUserMongoRepository(UserMongoRepository decorated, IDistributedCache distributedCache)
        {
