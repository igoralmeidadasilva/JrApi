...
namespace JrApi.Infrastructure.Repository
{
    ...
    public sealed class UserRepository : IDbRepository<UserModel> //FEEDBACK: not entirely necessary, but it's a fancy thing to always set as 'sealed' classes you know you wont inherit from.
    {
        ...
        public async Task<UserModel> Update(UserModel userUpdate) //FEEDBACK: you don't need the id argument here, because the UserModel already implements an Id. You can just use the Id from the user argument, or use the int argument. Choose between both.
        {
            ...
            //_dbContext.Entry(userUpdate).State = EntityState.Modified;
